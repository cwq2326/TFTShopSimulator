...
const poolSlice = createSlice({
    ...
    reducers: {
        removeChampionFromPool(state, action) {
            ...
            state.pool[tier - 1][index] = {
                ...champion,
                quantity: champion.quantity - 1,
            };
        },
        addChampionToPool(state, action) {
            ...
            state.pool[tier - 1][index] = {
                ...champion,
                quantity: champion.quantity + quantityToAdd,
            };
        },
        refreshShop(state, action) {
            ...
            state.shop.forEach((unit) => {
                if (unit) {
                    ...
                    const unitPoolIndex = getUnitPoolIndex(
                        unitTierIndex + 1,
                        unit
                    );
                    ...
                    state.pool[unitTierIndex][unitPoolIndex] = {
                        ...unit,
                        quantity:
                            state.pool[unitTierIndex][unitPoolIndex].quantity +
                            1,
                    };
                }
            });
            ...
            while (state.shop.filter((x) => x === null).length !== 0) {
                ...
                const unitIndex = Math.floor(
                    Math.random() * selectedPool.length
                );
                const selectedUnit = selectedPool[unitIndex];

                if (selectedUnit.quantity > 0) {
                    const shopIndex = state.shop.findIndex(
                        (unit) => unit === null
                    );

                    selectedPool[unitIndex] = {
                        ...selectedUnit,
                        quantity: selectedUnit.quantity - 1,
                    };
